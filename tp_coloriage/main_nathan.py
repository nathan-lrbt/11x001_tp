from utils import (
    afficher_coloriage,
    charger_coloriage,
    creer_coloriage_aleatoire,
    obtenir_couleur_aleatoire,
)
...
def colorier_pixel(image, i, j, couleur):
    if (
        image[i][j] != BLANC
    ):  # on ne veut colorier le pixel (i, j) seulement s'il est blanc
        return
...
def colorier_compartiment(image, i, j, couleur):
    ...
    if i < 0 or j < 0 or i >= len(image) or j >= len(image[0]) or image[i][j] != BLANC:
        ...
        # elif image[i + 1][j + 1] == BLANC:
        #     colorier_compartiment(image, i + 1, j + 1, couleur)
        # elif image[i + 1][j - 1] == BLANC:
        #     colorier_compartiment(image, i + 1, j - 1, couleur)
        # elif image[i - 1][j + 1] == BLANC:
        #     colorier_compartiment(image, i - 1, j + 1, couleur)
...
def exercice1():
    ...
    # ******************** Votre code ci-dessous ********************
    colorier_pixel(image, 20, 25, (255, 0, 0))
    # ******************** Votre code ci-dessus *********************
...
def exercice3():
    ...
    image = (
        creer_coloriage_aleatoire()
    )  # Décommenter cette ligne pour avoir un coloriage aléatoire à la place
    # ******************** Votre code ci-dessous ********************
    for i in range(len(image)):
        for j in range(len(image[0])):
            colorier_compartiment(image, i, j, obtenir_couleur_aleatoire())
    # ******************** Votre code ci-dessus *********************
    afficher_coloriage(image)
